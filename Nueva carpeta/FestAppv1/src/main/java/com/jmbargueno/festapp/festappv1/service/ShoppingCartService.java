...
public class ShoppingCartService {
	...
	private Map<PurchaseLine, Integer> purchaseLines = new HashMap <> ();
	...
	public void addPurchaseLine (PurchaseLine p) {
		if (purchaseLines.containsKey(p)) {
			purchaseLines.replace(p, purchaseLines.get(p)+1);//Ya programamos como "mayores" y podemos poner algún número directamente en el código
		}else {
			purchaseLines.put(p, 1);
		}
	}
	...
	public void removePurchaseLine (PurchaseLine p) {
        if (purchaseLines.containsKey(p)) {
            if (purchaseLines.get(p) > 1)
            	purchaseLines.replace(p, purchaseLines.get(p) - 1);
            else if (purchaseLines.get(p) == 1) {
    ...
    public Map<PurchaseLine, Integer> getProductsInCart() {
        return Collections.unmodifiableMap(purchaseLines);
    }
